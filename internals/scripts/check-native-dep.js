...
if (dependencies) {
  ...
  try {
    ...
    const filteredRootDependencies = rootDependencies.filter(
      (rootDependency) => !falseNatives.includes(rootDependency) && dependenciesKeys.includes(rootDependency),
    );
...
${chalk.whiteBright.bgGreen.bold('cd ./build/app && npm install your-package')}`);
      process.exit(1);
    }
  } catch (e) {
